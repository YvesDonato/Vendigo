...
test("manual inventory editing persists and updates an open shop; sold-out purchases are rejected", async ({ page, request }) => {
  ...
  await shop.goto("/shop");
  await expect(page.getByLabel("Coca-Cola", { exact: true })).toBeVisible();
  await page.getByLabel("Coca-Cola", { exact: true }).fill("5");
  await page.getByRole("button", { name: "Save Inventory", exact: true }).click();
  await expect(page.getByText("Inventory saved.")).toBeVisible();
  await expect(shop.getByTestId("product-coke")).toContainText("5 left");
  await page.reload();
  await expect(page.getByLabel("Coca-Cola", { exact: true })).toHaveValue("5");
  await page.getByLabel("Coca-Cola", { exact: true }).fill("0");
  await page.getByRole("button", { name: "Save Inventory", exact: true }).click();
  await expect(shop.getByTestId("product-coke").getByRole("button")).toBeDisabled();
  await expect(shop.getByTestId("product-coke")).toContainText("Sold out");
  const denied = await request.post("/api/purchases", { data: { orderId: "sold-out-api", robotId: "robot-001", productId: "coke", compartmentId: 1, sessionId: "sold-out-test" } });
  expect(denied.status()).toBe(409);
  const invalid = await request.post("/api/inventory", { data: { robotId: "robot-001", items: [{ productId: "coke", stock: -1 }] } });
