...
import { ArrowUpRight, BatteryMedium, Camera, Check, Home, LoaderCircle, LockKeyhole, MapPin, Package, Pause, Play, UnlockKeyhole } from "lucide-react";
import { api } from "@/lib/client";
import type { AppSnapshot, Robot, RobotCommand } from "@/types";
import type { CameraStatus } from "@/components/camera/live-camera";

const statusLabels = { available: "Selling", selling: "Serving a customer", stopped: "Stopped", returning: "Returning to base" };
...
export function RobotCard({ robot, state, cameraStatus }: { robot: Robot; state: AppSnapshot; cameraStatus: CameraStatus }) {
  ...
  async function command(value: RobotCommand) {
    ...
    try {
      ...
      setMessage(value === "stop" ? "Hawk is safely stopped." : value === "resume" ? "Hawk is ready to sell." : value === "return-to-base" ? "Return to base started." : value === "retry-lock" ? "Lock retry sent." : `Compartment ${compartment} opened. Auto-locks in 10 seconds.`);
  ...
  return <section className="panel robot-panel" id="robot">
    <div className="panel-heading"><div><span className="eyebrow">YOUR STORE ON WHEELS</span><h2>{robot.name}</h2></div><span className={`status-badge ${robot.status === "stopped" || robot.status === "returning" ? "badge-amber" : ""}`}><span className="status-dot" />{statusLabels[robot.status]}</span></div>
    <div className="robot-card-art"><div className="robot-card-grid" /><Image src="/robot.svg" alt="Hawk #1 autonomous vending robot" width={340} height={216} /><span className="robot-id">{robot.id.toUpperCase()} <ArrowUpRight size={12} /></span></div>
    <div className="robot-location"><MapPin size={15} /><span>{location.name}</span><span className="muted">Zone 01</span></div>
    <div className="robot-vitals"><div><BatteryMedium size={17} /><span>Battery</span><strong>{robot.battery}%</strong></div><div><Package size={16} /><span>Inventory</span><strong>{percent}%</strong></div><div><Camera size={16} /><span>Camera</span><strong className={cameraStatus === "online" ? "text-green" : "muted"}>{cameraStatus === "online" ? "Online" : cameraStatus === "connecting" ? "Connecting" : "Offline"}</strong></div></div>
    <div className={`compartment-state ${active ? "door-open" : ""}`} data-testid="compartment-state">{active ? <UnlockKeyhole size={15} /> : <LockKeyhole size={15} />}<span>{active ? `Compartment ${active.compartmentId} · ${active.status === "lock_failed" ? "needs lock retry" : active.status === "locking" ? "relocking" : active.status === "opening" ? "opening" : "unlocked"}` : "All compartments locked"}</span>{!active && <Check size={13} />}</div>
